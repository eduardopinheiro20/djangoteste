...
class CartItemManager(models.Manager): # Criando carrinho de compras

    def add_item(self, cart_key, product):
        if self.filter(cart_key=cart_key, product=product).exists(): # SE JA EXISTE NÃO VOU CRIAR UM NOVO
            created = False # NÃO CRIA UM NOVO PRODUTO
            cart_item = self.get(cart_key=cart_key, product=product) # AI ELE PEGA O ID E O PRODUTO
            cart_item.quantity = cart_item.quantity + 1 # SÓ AUMENTA A SUA QUANTIDADE E SOMA MAIS 1
            cart_item.save() # E SALVA O PRODUTO
        else:
            created = True # SE NÃO ESTIVER, AI SIM ADCIONA UM PRODUTO NO CARRINHO
